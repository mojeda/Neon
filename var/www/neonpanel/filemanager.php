...
include('./includes/loader.php');
	...
	if($LoggedIn === false){
		...
		if($sValidate->ValidatePath($sUser->sRootDir)){
			...
			$sError = "There seems to be a problem with your request. Please go back and try again.";
		}
...
".$sPostContent);
		...
		if($sAction == add_folder){
				$sFolderName = preg_replace("/[^a-z0-9_ .-]/i", "", $_GET['name']);
		...
		if($sAction == add_file){
			$sFileName = preg_replace("/[^a-z0-9_ .-]/i", "", $_GET['name']);
			if(!empty($sFileName)){
